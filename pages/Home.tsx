    ...
    <div className="min-h-screen px-6 pt-12">
      ...
      <div className="space-y-6">
        {letters.map((letter) => (
          ...
          >
            <div className={`absolute inset-0 ${letter.is_eternized ? 'bg-gradient-to-r from-cozy-sage/30 to-cozy-sand/30' : 'bg-cozy-sageLight/20'} opacity-40 rounded-[2rem] transform translate-y-2 group-hover:translate-y-3 transition-transform`}></div>
            <div className={`relative p-6 rounded-[2rem] shadow-soft border group-hover:-translate-y-1 transition-transform duration-300 ${letter.is_eternized ? 'bg-white/95 border-cozy-sage/30' : 'bg-white border-cozy-sageLight/20'}`}>
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-2">
                  {letter.is_eternized ? (
                    <div className="p-2 rounded-full bg-cozy-sage/20 text-cozy-sage animate-pulse-slow">
                      <Sparkles size={16} />
                    </div>
                  ) : (
                    <div className={`p-2 rounded-full ${letter.likes_count > 0 ? 'bg-cozy-clay/20 text-cozy-clay' : 'bg-cozy-sand/20 text-cozy-sand'}`}>
                      <Mail size={16} />
                    </div>
                  )}
                  <span className="text-xs font-bold text-cozy-sageDark tracking-wider">
              ...
              <h2 className="text-2xl font-serif text-cozy-deep mb-1">
                de {letter.author_name}
              </h2>
              <p className="text-xs text-cozy-sageDark mb-3">para {letter.recipient_name}</p>
              <p className="text-cozy-sageDark/80 line-clamp-2 font-serif italic">
